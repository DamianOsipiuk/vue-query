Vue Query provides three different network modes to distinguish how [Queries](guides/queries) and [Mutations](guides/mutations.md) should behave if you have no network connection. This mode can be set for each Query / Mutation individually, or globally via the query / mutation defaults.

Since Vue Query is most often used for data fetching in combination with data fetching libraries, the default network mode is [online](guides/network-mode#network-mode-online).
