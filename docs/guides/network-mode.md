Vue Query provides three different network modes to distinguish how [Queries](./queries) and [Mutations](./mutations.md) should behave if you have no network connection. This mode can be set for each Query / Mutation individually, or globally via the query / mutation defaults.

Since Vue Query is most often used for data fetching in combination with data fetching libraries, the default network mode is [online](./network-mode#network-mode-online).
